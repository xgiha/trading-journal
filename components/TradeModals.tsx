...
// --- Helper: Upload Logic (Cloud API Integration) ---
const uploadImageToBlob = async (file: File) => {
    ...
    try {
        ...
        console.warn("Upload record failed, but keeping local reference", e);
    }
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-5 custom-scrollbar pb-24 relative">
          <form id="entry-form" onSubmit={handleSubmit} className="grid grid-cols-2 gap-3 relative">
            <div className="relative bg-[#1E1E1E] rounded-2xl p-3 flex flex-col gap-1 focus-within:ring-1 focus-within:ring-nexus-accent h-[72px]">
             ...
             <div className="relative bg-[#1E1E1E] rounded-2xl p-3 flex flex-col gap-1 focus-within:ring-1 focus-within:ring-nexus-accent h-[72px]">
                 ...
                 <div className="flex items-center gap-1 h-full">
                     <span className={`text-lg font-mono font-light ${
            ...
            <div className="col-span-2 bg-[#1E1E1E] rounded-2xl p-1 flex relative isolate h-[56px]">
                ...
                <button
                  ...
                  className={`flex-1 flex flex-col items-center justify-center gap-0.5 rounded-xl z-10 ${
                    formData.direction === 'Long' ? 'text-green-400' : 'text-[#666]'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 flex flex-col items-center justify-center gap-0.5 rounded-xl z-10 ${
                    formData.direction === 'Short' ? 'text-red-400' : 'text-[#666]'
                  }`}
                >
            ...
            <div className="col-span-2 relative bg-[#1E1E1E] rounded-2xl p-3 flex flex-col gap-1 focus-within:ring-1 focus-within:ring-nexus-accent h-[72px]">
                 ...
                 <input 
                   ...
                   placeholder="e.g., VWAP Rejection"
                   id="strategy"
            ...
            <div className="col-span-2 bg-[#1E1E1E] rounded-2xl p-3 flex flex-col gap-1 h-[72px]">
                ...
                <textarea 
                    className="w-full h-full bg-transparent text-sm text-white focus:outline-none resize-none custom-scrollbar"
            ...
            <div className="col-span-2 bg-[#1E1E1E] rounded-2xl p-2 flex items-center gap-3 h-[72px]">
                 ...
                 <div className="flex-1 flex gap-2 overflow-x-auto custom-scrollbar items-center">
                     ...
                     <div 
                         ...
                         className={`w-10 h-10 rounded-lg border border-dashed border-white/20 flex flex-col items-center justify-center bg-white/[0.02] ${isUploading ? 'opacity-50' : 'cursor-pointer text-[#666]'}`}
                     >
                     ...
                     {formData.images.map((img, idx) => (
                         <div key={idx} className="w-10 h-10 rounded-lg relative shrink-0 group overflow-hidden bg-black/40">
                             ...
                             <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                 <button type="button" onClick={() => removeImage(idx)} className="text-red-500"><Trash2 size={12} /></button>
                             </div>
                         </div>
                     ))}
                 </div>
        ...
        <div className="absolute bottom-6 left-0 right-0 flex justify-center z-20 pointer-events-none">
          <button 
            ...
            className="pointer-events-auto relative px-8 py-4 rounded-full text-white bg-white/[0.02] hover:bg-white/[0.08] transition-all shadow-xl font-bold text-xs uppercase tracking-wider disabled:opacity-50"
          >
        ...
        >
                 ...
                 <div>
                     ...
                     <div className="flex gap-4 mt-2">
                         ...
                         <div className="flex flex-col">
                             ...
                             <span className="text-lg font-mono text-white">{winRate.toFixed(0)}%</span>
                         </div>
                     </div>
                 </div>
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar px-4 pb-20 pt-4 flex flex-col gap-2">
                 {sortedTrades.length === 0 ? (
                     <div className="py-10 text-center text-[#555] text-xs uppercase font-bold">No trades recorded</div>
                 ) : (
                     sortedTrades.map(trade => (
                         <div key={trade.id} className="bg-[#1E1E1E] rounded-xl p-3 flex flex-col gap-3">
                             <div className="flex items-center gap-3">
                                 <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${trade.type === 'Long' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                                 ...
                                 <div className="flex-1">
                                     <div className="flex justify-between items-center">
                                         ...
                                         <span className={`font-mono font-bold text-sm ${trade.pnl >= 0 ? 'text-green-500' : 'text-red-500'}`}>{trade.pnl >= 0 ? '+' : ''}{trade.pnl}</span>
                                     </div>
                                 ...
                                 <div className="flex gap-1">
                                     {onEdit && <button onClick={() => { onClose(); onEdit(trade); }} className="p-2 text-[#666]"><Edit2 size={14} /></button>}
                                     {onDelete && <button onClick={() => onDelete(trade.id)} className="p-2 text-[#666]"><Trash2 size={14} /></button>}
                                 </div>
                             </div>
                             {trade.images?.map((img, i) => (
                                 <img key={i} src={img} className="h-16 w-16 object-cover rounded-lg cursor-zoom-in" onClick={() => setZoomedImage(img)} />
                             ))}
                         </div>
                     ))
                 )}
             ...
             <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[#141414] to-transparent pt-10 flex justify-center pointer-events-none">
                 <button onClick={onAddTrade} className="pointer-events-auto flex items-center gap-2 bg-nexus-accent text-black px-6 py-3 rounded-full font-bold text-xs uppercase tracking-wider">

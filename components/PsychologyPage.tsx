...
const PsychologyPage: React.FC<PsychologyPageProps> = ({ trades, onBack }) => {
  ...
  // Helper to format the AI response into visual blocks
      ...
      <div className="flex flex-col gap-5 font-sans">
        {sections.map((part, i) => {
          ...
          if (trimmed === 'DIAGNOSIS' || trimmed === 'FINANCIAL IMPACT' || trimmed === 'COGNITIVE PRESCRIPTION') {
            return (
              <h4 key={i} className="text-[10px] font-bold text-purple-400 uppercase tracking-[0.2em] -mb-4 mt-1">
                {trimmed}
              </h4>
            );
          }
          if (trimmed.length > 0) {
            ...
            return (
              <p key={i} className={`${isDiagnosis ? 'text-lg md:text-xl font-light text-white tracking-tight leading-snug' : 'text-xs md:text-sm text-nexus-muted leading-relaxed'}`}>
                {trimmed}
              </p>
            );
          }
        ...
        <div className="lg:col-span-7 flex flex-col gap-4 md:gap-6 min-h-0">
          ...
          <div className="flex-1 bg-[#111111] rounded-[2rem] p-6 border border-white/5 flex flex-col min-h-0 shadow-xl overflow-hidden relative">
            ...
            <div className="flex-1 flex flex-col md:flex-row gap-6 min-h-0 z-10">
               ...
               {/* Right: Neural Stability Hub */}
               <div className="w-full md:w-60 shrink-0 flex flex-col items-center justify-center p-4">
                  ...
                  <div className="relative w-48 aspect-square flex items-center justify-center group">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-4xl font-light text-white tracking-tighter leading-none">{stats.score}%</span>
                        <span className="text-[8px] text-nexus-muted uppercase font-bold tracking-widest mt-1.5 flex items-center gap-1">
                            Alignment
                        </span>
                      </div>
                  </div>
                  ...
                  <div className="mt-8 w-full flex flex-col items-center gap-2">
                     <div className={`px-4 py-1 rounded-full border bg-white/5 flex items-center gap-2 transition-colors ${
                        ...
                        <span className="text-[9px] uppercase font-bold tracking-widest">
                           {stats.score > 70 ? 'Optimal Stability' : stats.score > 40 ? 'Moderate Noise' : 'System Degraded'}
                        </span>
                     </div>
                     <p className="text-[8px] text-nexus-muted uppercase font-bold tracking-[0.2em] opacity-40">State Analysis</p>
                  </div>
               </div>
            </div>
          </div>
